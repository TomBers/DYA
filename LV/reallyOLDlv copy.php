...
foreach ($codes as &$cde){
	$sql  = "SELECT qType FROM questions where code ='$cde';";
	$result = mysqli_query($db,$sql);
	while ($row = mysqli_fetch_assoc($result)) {
		$typ = ($row['qType']);
	}
	if($typ == 'CMMT'){
		echo "<li><div class=\"cmmt\" id=\"$cde\"><div>";

	}
	else if($type == 'SLD'){
	 echo "<li> <iframe scrolling=\"no\" id=\"frame\" src=\"http://162.13.147.58:8000/?code=$cde&slider=true\"></iframe>";
	}
	else{
	echo "<li> <iframe scrolling=\"no\" id=\"frame\" src=\"http://162.13.147.58:8000/?code=$cde\"></iframe>";
	}
}
echo "</ul>";	
